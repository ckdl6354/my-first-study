...
| :--- | :---: | :---: | :---: | :---: |
| **ArrayList** | 허용 | 유지 | 조회 $O(1)$ | 인덱스로 빠른 조회가 필요할 때 |
| **HashSet** | 불가 | 없음 | 탐색 $O(1)$ | 값의 존재 여부 확인, 중복 제거 |
| **HashMap** | Key 불가 | 없음 | 탐색 $O(1)$ | Key-Value 쌍 관리, 빈도수 카운팅 |
| **TreeSet / Map** | 불가 | **정렬** | 탐색 $O(\log N)$ | 최댓값·최솟값 검색, 실시간 정렬 |
