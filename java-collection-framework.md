...
| 자료구조 | 중복 여부 | 순서 유지 | 처리 속도 | 추천 시나리오 |
| :--- | :---: | :---: | :---: | :---: |
| **ArrayList** | 허용 | 유지 | 매우빠름 | 인덱스로 빠른 조회가 필요할 때 |
| **HashSet** | 불가 | 없음 | 매우빠름 | 값의 존재 여부 확인, 중복 제거 |
| **HashMap** | Key 불가 | 없음 | 매우빠름 | 빠른 Key-Value 조회 |
| **TreeSet / Map** | 불가 | **정렬** | 보통 | 범위 검색, 실시간 정렬 |
...
### List (순서 O, 중복 O)
*   **ArrayList**: 내부 배열 기반. 인덱스 조회가 빠르지만, 중간 삽입/삭제는 데이터 이동 때문에 느림. **(코테/실무 기본 리스트)**
*   **LinkedList**: 노드 연결 기반. 삽입/삭제가 빠르지만 탐색이 느림. **(Queue, Deque 구현 시 사용)**
